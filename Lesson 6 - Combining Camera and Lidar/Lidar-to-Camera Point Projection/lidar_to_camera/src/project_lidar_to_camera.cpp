...
void projectLidarToCamera2()
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        ...
        // Filter lidar points (behind the Lidar, too far in x or y direction, too close to the road surface in Z direction, with low reflectivity)
        float xMax = 25.0, yMax = 6.0, zMin = -1.4;
        if (it->x < 0.0 || it->x > xMax || abs(it->y) > yMax || it->z < zMin || it->r<0.01) 
            continue;
        ...
        float val = it->x;
        float maxVal = 20.0;
        int green = min(255, (int) (abs(val) / maxVal * 255));
        int red = 255 - green;
